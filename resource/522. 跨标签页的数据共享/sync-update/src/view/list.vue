...
const unListen = listenMsg((info) => {
  if (info.type === 'add-imp') {
    emps.value.unshift(info.msg);
  } else if (info.type === 'update-imp') {
    const index = emps.value.findIndex((item) => item.id === info.msg.id);
    emps.value.splice(index, 1, info.msg);
  }
});
